...
window.addEventListener('DOMContentLoaded', () => {
    let timeLeft = 20;
    var timers = document.querySelector('#time');
...
const myTimer = setInterval(() => {
    ...
    // 40 seconds
    const minsLeft = (timeLeft - secondsLeft)/60
    
    const timeString = `${minsLeft}:${secondsLeft < 10 ? '0' : ''}${secondsLeft}`;
    ...
    const calculateScore = () => {
      ...
      const scoreh3 = document.getElementById('score')
       scoreh3.innerHTML = `your score is ${score}`
    };
...
//     log.textContent = `quiz Submitted! Time stamp: ${event.timeStamp}`;
//     event.preventDefault();
//   }
  
//   const submit = document.getElementById('btnSubmit');
//   const Resetquiz = document.getElementById('btnReset');
